...
m.views.Background = Backbone.View.extend({
    ...
    render: function () {
        ...
        $('<img/>').attr('src', 'https://momentumdash.com/backgrounds/' + filename).load(function() {
            that.$el[order]('#' + that.options.region).css('background-image','url(https://momentumdash.com/backgrounds/' + filename + ')').css('opacity','0').fadeTo(200, 1);
            $(this).remove();
        });
